...
spinlock_t kernel_heap_lock = init_spin(kernel_heap_lock);
...
void placement_kfree(void*ptr ATTR((unused)))
{
	syslog(KERN_WARN, "kfree called too early.");
}

// #define STEWIEOS_HEAP_MAGIC 0x2BADB000
...
// 			syslog(KERN_ERR, "kmem: bad magic number. discarding block at %p.", entry);
// 			list_rem(&entry->link);
// 			return heap_find_hole(size, align);
// 		}
...
// static void heap_insert_hole(struct header* header)
// {
// 	if( ((u32)header) < heap_data->startptr || ((u32)header) >= heap_data->endptr ){
// 		syslog(KERN_ERR, "attempted to insert a hole outside the heap (address: 0x%p).", header);
// 	}
...
// 		struct header* entry = list_entry(item, struct header, link);
// 		if( entry->magic != STEWIEOS_HEAP_MAGIC ){
// 			syslog(KERN_ERR, "discarding bad heap hole (address: 0x%p).", entry);
// 			list_rem(item);
// 			heap_insert_hole(header);
// 			return;
// 		}
...
// 	if(!header)
// 	{
// 		syslog(KERN_PANIC, "kernel: out of memory!");
...
// 		syslog(KERN_ERR, "corrupted kernel heap (address: 0x%p).", header);
// 		return;
// 	}
// 	// not a hole
...
// 		syslog(KERN_PANIC, "kernel heap full. out of memory!");
// 		return NULL;
// 	}
...
// 		syslog(KERN_ERR, "corrupted kernel heap (address: 0x%p).", prev_foot);
// 		return header;
// 	}
// 	struct header* prev_head = prev_foot->header;
// 	// previous block is invalid, can't safely merge
// 	if( prev_head->magic != STEWIEOS_HEAP_MAGIC ){
// 		syslog(KERN_ERR, "corrupted kernel heap (address: 0x%p).", prev_head);
// 		return header;
// 	}
