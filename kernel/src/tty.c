...
int tty_file_open(struct file* file, struct dentry* dentry __attribute__((unused)), int mode __attribute__((unused)))
{
	...
	if( tty_driver[major]->open && device->refs == 1 )
	{
		int result = tty_driver[major]->open(tty_driver[major], device);
		if( result != 0 ){
			device->refs--;
			if( device->refs == 0 ){
				device->task = NULL;
...
ssize_t tty_file_write(struct file* file, const char* buffer, size_t count)
{
	...
	if( device->ops->write ){
		return (ssize_t)device->ops->write(device, buffer, count);
	}
...
int tty_file_ioctl(struct file* file, int cmd, char* arg)
{
	...
	if( device->ops->ioctl ){
		return (ssize_t)device->ops->ioctl(device, cmd, arg);
	}
